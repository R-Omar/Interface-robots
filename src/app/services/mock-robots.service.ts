...
export class MockRobots {
  ...
  simulateRobotsBehavior(): IRobot[] {
    this.robots
      ...
      .forEach((robot) => {
        let executionTimeKPI = robot.KPIs.filter((KPI) => KPI.label === "Temps d'exécution")[0];
        ...
        if(totalSeconds %5 === 0)
          (Math.floor(Math.random() * 10) < 9 ) ? prisesKPI.valeur += 1 : erreursKPI.valeur += 1;
      });
    return this.robots;
  }
}
